...
```text
<UnityProject>/Locus/views/<view-id>/
```
...
- C# `ViewApi.cs` 示例：读取 Unity 数据、写回 Unity 数据。
...
```ts
import { defineView, useViewState, useUnityBinding, useViewScript } from "@locus/view-runtime";
...
```text
/view <需求>
1. 判断 View 类型：blank / inspector-form / node-graph / link-board / scripted-transform / readonly-dashboard
2. 加载必要 view_* 工具
